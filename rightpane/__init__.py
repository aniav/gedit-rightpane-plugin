...
class RightPanePluginInstance:
    ...
    def transfer_tab(self, from_pane, to_pane, item):
        """
        Transfer a tab between panes
        """
        self.lock = True
        item.reparent(to_pane)
        self.lock = False
    ...
    def set_right_activated_tab(self):
        ...
        if activated_item and len(notebook.get_children()) > 1:
            self.config.set('right_pane', 'tab-active',
                            notebook.get_menu_label_text(activated_item))
        else:
    ...
    def set_left_pane_visibility(self):
        ...
        """
        checkbox = self.window.get_ui_manager()\
                       .get_widget("/MenuBar/ViewMenu/ViewSidePaneMenu")
        self.config.set('left_pane', 'visible', str(checkbox.get_active()))
...
#        self.action_group = None # Ugly but can cause a seg fault...
    ...
    def insert_menu(self):
        ...
        self.action_group.add_toggle_actions([("ViewRightSidePane", None,
                                              _("Right Side Pane"), "<Ctrl>F8",
                                              _("Right Pane"),
                                              self.display_right_pane)])
        managerightpane_action = Gtk.Action(name="ManageRightSidePane",
    ...
    def restore_left_pane_visibility(self):
        ...
        """
        if self.config.has_option('left_pane', 'visible') \
            and not self.config.getboolean('left_pane', 'visible'):
            chk = self.window.get_ui_manager().get_widget("/MenuBar/ViewMenu/ViewSidePaneMenu")
            chk.set_active(False)
            self.left_pane.set_property("visible", False)

    def display_right_pane(self): #FIX args, as unneeded!
        """
    ...
    def build_popup(self):
        ...
        #self.popup.set_type_hint(Gdk.DECOR_BORDER)
